class IsolateJob < ApplicationJob
  ...
  def extract_archive
    ...
    #{cgroups.present? ? "-p#{Config::MAX_MAX_PROCESSES_AND_OR_THREADS}" : ""} \
    #{submission.enable_per_process_and_thread_memory_limit || cgroups.blank? ? "-m " : "--cg-mem="}#{Config::MAX_MEMORY_LIMIT} \
  ...
  def run
    ...
    #{cgroups.present? ? "-p#{submission.max_processes_and_or_threads}" : ""} \
    #{submission.enable_per_process_and_thread_memory_limit || cgroups.blank? ? "-m " : "--cg-mem="}#{submission.memory_limit} \
